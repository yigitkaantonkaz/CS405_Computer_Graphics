...
class MeshDrawer {
	...
	constructor() {
		...
		this.baseTextureLoc = gl.getUniformLocation(this.prog, 'baseTexture');
    	this.normalMapLoc = gl.getUniformLocation(this.prog, 'normalMap');
	...
	draw(trans) {
		...
		gl.uniform1f(this.ambientLoc, ambientValue);

		const specularValue = parseFloat(document.getElementById('specular-light-setter').value) / 100;
		gl.uniform1f(this.specularLoc, specularValue);

		updateLightPos();
		gl.uniform3fv(this.lightPosLoc, [lightX, lightY, 10.0]);

		const enableLight = document.getElementById('enable-light').checked;
		gl.uniform1i(this.enableLightingLoc, enableLight ? 1 : 0);

		gl.uniform1i(this.useNormalMapLoc, this.normalMap ? 1 : 0);
	...
	setTexture(img, isNormalMap = false) {
		...
		if (this.baseTexture) {
			...
			gl.uniform1i(this.baseTextureLoc, 0);
		}
		if (this.normalMap) {
...
const meshFS = `
			...
			void main()
			{
				...
				if (useNormalMap) {
					vec3 normalFromMap = texture2D(normalMap, v_texCoord).rgb * 2.0 - 1.0;
